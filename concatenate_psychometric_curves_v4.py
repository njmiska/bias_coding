...
for j in range(0,np.size(eids)):
    ...
    ########################################################################################
        ...
        for trial_number, condition_number in stimtrial_location_dict_all.items():
            ...
            if align_to == 'goCue':
                start_time = trials.goCueTrigger_times[trial_number] #GO CUE
            elif align_to == 'goCue_pre':
                start_time = trials.goCueTrigger_times[trial_number] - 0.5 #GO CUE - 0.5s
            elif align_to == 'QP':
            ...
            for l in range(int(length_of_time_to_analyze_wheel_movement/interval)):
                ...
                ##only in case where aligning to QP, do not use any wheel movement past goCue
                if align_to == 'QP' and trials.goCueTrigger_times[trial_number] < whlt[wheel_end_index]:
                    total_wheel_movement = np.append(total_wheel_movement,np.NaN)
                ##only in case where aligning to goCue, do not use any wheel movement past feedback + 0.1
...
if is_zapit_session == 0:
    ...
    # Set limits and labels
    ax1.set(ylim=[-.05, 1.05], xlabel='contrast (%)', ylabel='proportion rightward')
...
####################### ZAPIT ANALYSIS HERE
    ...
    norm = mcolors.Normalize(vmin = RT_analysis_results[0]['mean'] - 0.4*RT_analysis_results[0]['mean'], vmax = RT_analysis_results[0]['mean'] + 0.6*RT_analysis_results[0]['mean'])
