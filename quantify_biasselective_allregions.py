...
# import sys
# sys.path.append('/Users/natemiska/Documents/python/functions')
# from ephys_functions_guido import paths, query_sessions
...
######### Version of BS analysis function with NO optogenetic modification
def isbiasblockselective_04(current_unit_spike_times, trials_probabilityLeft, trials_goCue_times,
    ...
    pseudo_pvals = np.empty(num_pseudo_blocks+1) # reserve an extra spot for the real pvalue
    pseudo_pvals[:] = np.NaN

    # The first entry in pseudo_pvals should be the real p-value
    pseudo_pvals[0] = pval_real

    # The rest of pseudo_pvals are p-values from pseudo blocks
    for k in range(1, num_pseudo_blocks+1):     
        fr_80_trials_pseudo = fr_per_trial[pseudo_80_index_filtered[k-1]]
        fr_20_trials_pseudo = fr_per_trial[pseudo_20_index_filtered[k-1]]
        pval_pseudo = ranksums(fr_80_trials_pseudo, fr_20_trials_pseudo).pvalue
        pseudo_pvals[k] = pval_pseudo

    # Perform the FDR correction on all p-values (real and pseudo)
    reject, fdr_pvals, _, _ = multipletests(pseudo_pvals, alpha=BS_pval_threshold, method='fdr_bh')

    # Extract the corrected p-value for the real data
    fdr_pval_real = fdr_pvals[0]

    # Calculate 5th percentile of corrected p-values
    percentile_5 = np.percentile(fdr_pvals[1:], 5)  # Calculate 5th percentile

    # Check if the real p-value is significant after FDR correction
    if fdr_pval_real < percentile_5:
        isbiasselective = 1
    else:
        isbiasselective = 0
    ...
    return isbiasselective, pval_real, fr_80_trials_nonstim, fr_20_trials_nonstim
...
aligned_pids_df.to_pickle('~/Documents/python/saved_figures/' + 'aligned_pids_df_current' + '.pkl')
...
num_units = 0
for k in np.arange(630,np.size(aligned_pids)):
    ...
    try:
        ...
        for j in region_indices: ### legacy code
            if j in spikes.clusters and clusters_labels[j] >= cluster_label_threshold and mean_firing_rates[j] > FR_threshold:
                ...
                fr_per_trial[:] = np.NaN
                fr_per_trial = fr_per_trial[0]
                isbiasselective, pval_real, fr_80_trials_nonstim, fr_20_trials_nonstim = isbiasblockselective_04(
                ...
                region_clusters_info.to_csv('~/Documents/python/saved_figures/' + 'bs_analysis_allregions_dataframe_V5.csv', index=False)
